...
description: Hardens frontend code against browser vulnerabilities. Use when handling user input in forms/UI, auth tokens, public env vars, XSS sinks, or dependency audits. Prefer `.cursor/references/security-checklist.md`. Do not implement server-side password hashing or SQL controls in this repo.
---
...
Focus on XSS, open redirects, secret leakage in `NEXT_PUBLIC_*` / `VITE_*`, token storage, route guards, and `pnpm audit`. Skip password hashing, SQL injection hardening, and DB encryption — those are backend concerns. Use `.cursor/references/security-checklist.md`.

## Overview

Security-first practices for the browser client. Treat every external input as hostile, every secret as sacred, and every authorization check in the UI as necessary but insufficient without API enforcement.
