...
description: Optimizes frontend performance — Core Web Vitals, bundles, rendering, and client caching. Use when CWV or load times need improvement, or profiling shows UI bottlenecks. Prefer `.cursor/references/performance-checklist.md`. Skip DB/N+1 backend work in this repo.
---
...
Measure LCP/INP/CLS, bundle size, images, fonts, and React render cost for `apps/www` and `apps/app`. Do not run DB query plans or Redis caching designs here. Use `.cursor/references/performance-checklist.md`.
