...
description: Guides stable client API and interface design for the frontend. Use when designing axios/React Query modules, TypeScript payload contracts, component props, or module boundaries. Do not implement backend servers here — consume and type external APIs only.
---
...
Apply this skill to **client** contracts only: `src/apis/*`, payload types, React Query hooks, error mapping, auth headers, and component/module boundaries. Do **not** implement REST servers, DB schemas, or migrations in this monorepo. When an upstream API is missing, document the needed contract and stop — do not invent a backend.

## Overview

Design stable, well-documented interfaces that are hard to misuse. Good interfaces make the right thing easy and the wrong thing hard. This applies to HTTP client modules, TypeScript contracts, module boundaries, component props, and any surface where one piece of code talks to another.

## When to Use

- Adding or changing client API modules (`apps/www` or `apps/app`)
- Defining TypeScript request/response contracts
- Creating component prop interfaces and shared module boundaries
- Negotiating contracts with the backend team (document; don't implement the server)
- Changing existing public UI or client interfaces
