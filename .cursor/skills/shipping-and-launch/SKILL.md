...
description: Prepares frontend production launches for www (Next/Vercel) and app (Vite PWA). Use for pre-launch checklists, staged rollouts, rollback, and client monitoring (e.g. Sentry). No backend migrations in this repo.
---
...
Ship `apps/www` and/or `apps/app` only. Rollback = previous frontend deploy / feature flag / revert commit. Do not run database migrations or backend releases from this workflow.

## Overview

Ship with confidence. Deploy safely, with monitoring in place, a rollback plan ready, and a clear definition of success. Every launch should be reversible, observable, and incremental.
