...
This persona reviews the **WithFestival frontend**. Skip backend-only controls (password hashing, SQL, DB backups). Use `.cursor/references/security-checklist.md`.

### 1. Input Handling
- Is user input validated at form/API client boundaries?
- Are XSS sinks avoided (`dangerouslySetInnerHTML`, unsanitized markdown)?
- Are file uploads restricted by type/size in the UI?
- Are URL redirects validated against an allowlist?

### 2. Authentication & Authorization (client)
- Are tokens stored and cleared deliberately (`sessionStorage` / logout / 401)?
- Are protected and admin routes guarded in the router (not only by hiding links)?
- Can the UI be tricked into calling privileged APIs without a token? (still note that the API must enforce auth)

### 3. Data Protection
- Are secrets kept out of `NEXT_PUBLIC_*` / `VITE_*` and source?
- Are tokens/PII excluded from logs, toasts, and analytics payloads?
- Is production traffic HTTPS/WSS?

### 4. Client / Deploy Hygiene
- Are CSP and related headers considered for production?
- Are dependencies audited (`pnpm audit`)?
- Are error UIs generic (no stack traces)?
