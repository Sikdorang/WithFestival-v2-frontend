...
### Integration
- [ ] Change works with the rest of the frontend system, not just in isolation (`www` vs `app` boundaries respected)
- [ ] Env/config changes and feature flags are accounted for (no backend DB migrations in this repo)
- [ ] Backward compatibility considered for shared client modules, public routes, and API response shapes the UI depends on

### Documentation
- [ ] Public interfaces, client APIs, and user-facing behavior are documented
...
### Ship-readiness
- [ ] Security implications reviewed for untrusted input, tokens, and XSS (see `security-and-hardening`)
- [ ] Client observability considered for critical paths where applicable (e.g. Sentry in `apps/app`)
...
- [ ] `pnpm lint` and the relevant app `build` succeed; tests pass when a harness exists for the change
